...
class TestDocumentLoadingPipeline:
    ...
    async def test_load_document_stores_metadata_and_comments(
        ...
        # Load document
        loader = DocumentLoader(database_url=database_url)
        ...
        assert doc_row["title"] == "Test Document"
        assert doc_row["object_id"] == object_id
        ...
        assert comment_rows[0]["id"] == "C1"
        assert comment_rows[0]["comment_text"] == "First comment"
        assert comment_rows[1]["id"] == "C2"
        assert comment_rows[1]["comment_text"] == "Second comment"
    ...
    async def test_load_document_skips_existing_comments(
        ...
        loader2 = DocumentLoader(database_url=database_url)
...
class TestCommentProcessingPipeline:
    ...
    async def test_process_comments_flow(
        ...
        # Process comments
        processor = CommentProcessor(database_url=database_url)
        ...
        assert rows[0]["category"] == "Physicians & Surgeons"
        assert rows[0]["sentiment"] == "for"
        assert rows[1]["category"] == "Physicians & Surgeons"
        assert rows[1]["sentiment"] == "for"
...
class TestFullEndToEndPipeline:
    ...
    async def test_full_pipeline_load_then_process(
        ...
        assert "physician" in comment_row["comment_text"].lower()
        assert comment_row["category"] == "Physicians & Surgeons"
        assert comment_row["sentiment"] == "for"
